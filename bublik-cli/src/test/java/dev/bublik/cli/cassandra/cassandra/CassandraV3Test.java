...
public class CassandraV3Test {
    private static int rows = 50000;
    private static boolean sync = false;

    private static CassandraContainer source = new CassandraContainer("cassandra:3.11.19")
            .withEnv("CASSANDRA_USER", "cassandra")
            .withEnv("CASSANDRA_PASSWORD", "cassandra")
            .withEnv("CASSANDRA_ENABLE_USER_DEFINED_FUNCTIONS", "true")
            .withEnv("CASSANDRA_AUTHENTICATOR", "PasswordAuthenticator")
            .withEnv("CASSANDRA_NUM_TOKENS", "16")
            .withCopyToContainer(MountableFile.forClasspathResource("./cassandra/cassandra/conf/docker-entrypoint-v3.sh"), "/usr/local/bin/docker-entrypoint.sh")
            .withInitScript("./cassandra/cassandra/sql/cs-init-v3.cql")
            .withExposedPorts(9042);
    private static CassandraContainer target = new CassandraContainer("cassandra:3.11.19")
            .withEnv("CASSANDRA_USER", "cassandra")
            .withEnv("CASSANDRA_PASSWORD", "cassandra")
            .withEnv("CASSANDRA_ENABLE_USER_DEFINED_FUNCTIONS", "true")
            .withEnv("CASSANDRA_AUTHENTICATOR", "PasswordAuthenticator")
...
// select id, uid, v1, v2, v3, v4, ttl(v1), ttl(v2), ttl(v3), ttl(v4), writetime(v1), writetime(v2), writetime(v3), writetime(v4)  from test.t1;
    public void allTypes() throws InterruptedException, IOException {
        ...
        TestResult result = getResult(
                "./cassandra/cassandra/yaml/cs2cs.yaml",
                "./cassandra/cassandra/json/cs2cs5V3.json",
                rows,
                sync,
                sourceProperties,
                targetProperties,
...
//        Thread.sleep(120_000);
    ...
    public void withTtlBasedOnColumn() throws InterruptedException, IOException {
        ...
        TestResult result = getResult(
                "./cassandra/cassandra/yaml/cs2cs.yaml",
                "./cassandra/cassandra/json/cs2cs8V3.json",
                rows,
                sync,
                sourceProperties,
                targetProperties,
