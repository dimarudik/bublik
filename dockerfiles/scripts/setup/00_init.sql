...
create table test.likes (
    ...
    last_update timestamp,
    primary key (id));
create index likes_user_item_uix on likes (user_id, item_id);
...
insert into test.users (id, user_name, email)
    ...
    from (
        select rownum as id, DBMS_RANDOM.STRING('X', 32) as user_name
        from dual connect by level < 100000
        );

insert into test.items (id, item_name, description)
    select rownum as id,
           'Item ' || DBMS_RANDOM.STRING('X', 32) as item_name,
           'Description ' || DBMS_RANDOM.STRING('X', 100) as description
    from dual connect by level < 100000;

insert into test.likes (id, user_id, item_id, r)
    select rownum as id,
       FLOOR(DBMS_RANDOM.VALUE(1, 100000)) as user_id,
       FLOOR(DBMS_RANDOM.VALUE(1, 100000)) as item_id,
